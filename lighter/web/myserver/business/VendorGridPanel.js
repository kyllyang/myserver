Ext.define('Business.VendorGridPanel', {
	...
	doDeleteEvent: function(ids) {
		...
		Ext.Ajax.request({
			url: ctx + '/business/vendor/delete.ctrl',
			params: {
				ids: validIds
			},
	...
	saveGrid: function() {
		...
		if(datas.length > 0) {
			Ext.Ajax.request({
				url: ctx + '/business/vendor/save.ctrl',
				params: {
					vendorJson: Ext.encode(datas)
				},
				success: function(response, opts) {
