Ext.define('Business.CustomerFormWindow', {
	extend: 'Ext.window.Window',

	entityId: null,
	customerGridPanel: null,
	readOnlyForm: false,
	...
	initComponent: function() {
		...
		if (this.entityId) {
			this.getComponent('editForm').getForm().load({
				url: ctx + '/business/customer/input.ctrl',
				params: {
					id: this.entityId
				},
				waitMsg: '正在载入数据...',
	...
	saveForm: function() {
		...
		if (form.isValid()) {
			form.submit({
				url: ctx + '/business/customer/save.ctrl',
				waitMsg: '正在保存数据，请稍候...',
				success: function(form, action) {
					Ext.Msg.alert('系统提示', '数据保存成功！');
					this.closeForm();
					this.customerGridPanel.queryData();
				},
