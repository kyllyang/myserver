Ext.define('Base.sys.config.ConfigGridPanel', {
	extend: 'Base.ux.GridPanel',

	initComponent: function() {
		Ext.apply(this, {
			itemId: 'configGridPanel',
			url: ctx + '/sys/config/list.ctrl',
			sortProperty: 'sort',
			sortDirection: 'asc',
			enableEditBtn: false,
			enableViewBtn: false,
	...
	doDeleteEvent: function(ids) {
		...
		Ext.Ajax.request({
			url: ctx + '/sys/config/delete.ctrl',
			params: {
				ids: validIds
			},
	...
	saveGrid: function() {
		...
		if(datas.length > 0) {
			Ext.Ajax.request({
				url: ctx + '/sys/config/save.ctrl',
				params: {
					configJson: Ext.encode(datas)
				},
				success: function(response, opts) {
